...
public class MainTestHashMap {
	public static void main(String[] args) {
		...
		try {
			writer = new FileWriter(".\\src\\com\\tj\\ex7_homework\\customerhashmap.txt");
			do {
				...
				if(str.equalsIgnoreCase("n")) {
					...
					// 종료할때 파일저장, 콘솔창에 출력
					iter = customers.keySet().iterator();
					while(iter.hasNext()) {
						String key = iter.next();
						System.out.println(customers.get(key));
						writer.write(customers.get(key).toString());
					}
				...
				// arraylist에 저장
				customers.put(phone, new Customer(name, phone, birth, address));
